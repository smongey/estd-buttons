...
for (const [name, schema] of Object.entries(window.estd.buttons.marks ?? {})) {
  marks[name] = {
    get button() {
      return {
...
for (const [name, schema] of Object.entries(window.estd.buttons.links ?? {})) {
  marks[name] = {
    get button() {
      return {
        icon: schema.icon,
...
window.panel.plugin("estd/buttons", {
  writerMarks: marks,
});
